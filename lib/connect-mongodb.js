...
var Store = require('connect').session.Store
  ...
  , _collection = null
  , _defaults = {host: '127.0.0.1', port: 27017, dbname: 'dev', collection: 'sessions', reapInterval: 60 * 1000, safe: false};
...
var MONGOSTORE = module.exports = function MongoStore(options, callback) {
  ...
  if (options.url) {
    ...
    , new mongo.Server(url.hostname || _defaults.host, +url.port || _defaults.port, {safe: _defaults.safe})
    );
  }

  if (options.server_config) {
    server_config = options.server_config;
    db = new mongo.Db(options.dbname || _defaults.dbname, server_config, {safe: _defaults.safe});
  }
